const String logo = 'assets/svg/logo.svg';

const String avatorOne = 'assets/svg/avator1.svg';
const String avatorTwo = 'assets/svg/avator2.svg';
const String avatorThree = 'assets/svg/avator3.svg';

const String mainBanner = 'assets/svg/bg_home.svg';
const String cashback = 'assets/svg/cashback.svg';
const String cloud = 'assets/svg/cloud.svg';
const String electricity = 'assets/svg/electricity.svg';
const String filter = 'assets/svg/filter.svg';
const String flight = 'assets/svg/flight.svg';
const String logout = 'assets/svg/logout.svg';
const String movie = 'assets/svg/movie.svg';
const String menu = 'assets/svg/menu.svg';
const String mobile = 'assets/svg/mobile.svg';
const String more = 'assets/svg/more.svg';
const String recive = 'assets/svg/receive.svg';
const String send = 'assets/svg/send.svg';
const String scan = 'assets/svg/scan.svg';
const String cross = 'assets/svg/cross.svg';
